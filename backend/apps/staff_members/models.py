...
class StaffMember(AbstractUser):
        ...
        def clean(self):
            ...
            # Enforce that branch managers and supervisors have a branch.
        ...
        def save(self, *args, **kwargs):
            # Call model validation first.
            self.full_clean()

            # If this user is a branch manager and a branch is assigned, update the branch's manager field.
            if self.role == self.Role.BRANCH_MANAGER and self.branch:
                if self.branch.manager != self:
                    self.branch.manager = self
                    self.branch.save()
            else:
                # If not a branch manager, clear any branch where this user is set as manager.
                from apps.branch_location.models import Branch
                for branch in Branch.objects.filter(manager=self):
                    branch.manager = None
                    branch.save()
