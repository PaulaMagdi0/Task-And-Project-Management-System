...
class TrackSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        # Extract courses from the data and remove them from the track data
        courses = validated_data.pop('courses', [])
        track = Track.objects.create(**validated_data)
        track.courses.set(courses)  # Add courses to the track (using set to manage many-to-many relationship)
        return track
    ...
    def update(self, instance, validated_data):
        ...
        for attr, value in validated_data.items():
            setattr(instance, attr, value)
        instance.save()
        instance.courses.set(courses)  # Update the courses in the many-to-many relationship
        return instance
