...
const ChatWithAI = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const { data } = await apiClient.post("/chatAI/", { message: input });
      ...
      setError(
        "Failed to get response: " + (err.response?.data?.error || err.message)
      );
      if (retryCount < maxRetries) {
        setTimeout(() => {
  ...
  return (
    <Box className="chat-container">
      <Card
        elevation={3}
        sx={{ borderTop: "4px solid #00897b", height: "100%" }}
      >
        <CardHeader
          title="Chat With AI"
